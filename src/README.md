...
```bash
data-visualiser$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
data-visualiser$ sam local start-api
data-visualiser$ curl http://localhost:3000/
```
...
```bash
data-visualiser$ sam logs -n HelloWorldFunction --stack-name data-visualiser --tail
```
...
```bash
sam delete --stack-name data-visualiser
```
